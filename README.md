...
Data frame **info** should include minimum one column for **grouplabels**, which are indicated with integers (0 for controls). If the dataset contains no sample groups, use dummy value of 0 for all samples. Other columns can be either numeric or character. Notably, **info** can include also other columns than those indicated by **grouplabel**, **neutralize**, and **mainfeature**. Variables (columns) not indicated by any of those arguments are used in model fitting, but their effect is not neutralised. Do not encode non-ordinal categorical variables with integers.
